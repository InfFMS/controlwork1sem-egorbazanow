...
price1 = int(input()) # 1. Цена покупки облигации.
price2 = int(input())# 2. Цена погашения облигации.
money = int(input())# 3. Сумму разовой купонной выплаты.
mesac=int(input())#4. Периодичность выплаты купонов (в месяцах).
procent=int(input())#5. Комиссию брокера (в процентах) — применяется только при покупке облигации.
T = int(input())#6. Период до погашения облигации (в месяцах).
comisiatobuy =price1*(procent/100) #комиссия на покупку
alterprice = price1 +comisiatobuy #полная цена покупки
cupon = money * (T/mesac) #общий купонный доход
athmoney = price2+cupon - alterprice #Общий доход до налогообложения
if athmoney >0:
    nalog = athmoney *0.13
else:
    nalog = 0     #Находим вычитаемый налог
chistdohod = (athmoney - nalog)/(price1 * 100) #Считаем чистый доход в процентах
print('Чистый доход составит:', chistdohod)
